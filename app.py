...
from src.service.vtex_service import VtexServiceAsync
from src.processor.vtex_processor import VtexProcessorAsync
...
if st.button("🚀 Iniciar Processamento", type="primary"):
    try:
        ...
        vtex_service = VtexServiceAsync(vtex_config)
        processor = VtexProcessorAsync()
        ...
        with st.spinner("Buscando pedidos na VTEX..."):
            ...
            all_summaries = asyncio.run(
                vtex_service.fetch_all_orders(dt_ini, dt_fim)
            )
        ...
        def up_bar(c, t): prog_bar.progress(c/t)
        
        data_rows = asyncio.run(
            processor.process_all(all_summaries, vtex_service, up_bar)
        )
